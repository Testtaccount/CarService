...
import am.gsoft.carservice.firebase.FirebaseApi.Path;
...
//        appSharedHelper.saveUser(user);
    ...
    mExecutors.diskIO().execute(new Runnable() {
      ...
      public void run() {
        if (App.getInstance().isSimulator) {
          mUser = mAppDatabase.mUserDao().get("oyTyrPcSmMeYdIAkyU1gOG8eAEe2");
        } else {
  ...
  public LiveData<Resource<User>> loadUser(String uId) {
    return new NetworkBoundResource<User, User>() {

      @Override
      protected void saveCallResult(@NonNull List<User> users) {
        mAppDatabase.mUserDao().insert(users.get(0));
      }
      ...
      protected LiveData<User> loadFromDb() {
        return mAppDatabase.mUserDao().getLive(uId);
      }

      @NonNull
      @Override
      protected Call<HashMap<String, User>> createCall() {
        return null;
      }

      @Override
      protected Query getDatabaseReference() {
        return mFirebaseApi.getUserDatabaseReference().child(uId);
      }
    }.getAsLiveData();
  }

  public LiveData<List<User>> getUsers() {
    return mAppDatabase.mUserDao().getAll();
  }
...
//                resultListenerListener.onLoad(mUser);
              }
            });
          }
    ...
    //Retrofit
//          mAppNetworkService.saveOil(carKey, mUser.getKey(), mUser).enqueue(new Callback<Oil>() {
//            @Override
...
//                  @Override
//                  public void run() {
//                    mAppDatabase.mOilDao().update(mUser);
//                    resultListenerListener.onComplate(mUser);
//                  }
//                });
//              } else {
...
//
//              list.set(indexOfCurrentOil,mUser);
//              appSharedHelper.setSaveOilListByCarKey(carKey,list);
//              carOilResultListener.onSuccess(mUser);
          }
    ...
    //Retrofit
//          mAppNetworkService.updateOil(carKey, mUser.getKey(), mUser)
...
//    });
  ...
  public void createService(Service service, ResultListener<Service> userResultListenerListener) {
    mFirebaseApi.getServiceDatabaseReference().child(service.getKey())
        .addListenerForSingleValueEvent(new ValueEventListener() {
          @Override
          public void onDataChange(DataSnapshot dataSnapshot) {
            mExecutors.diskIO().execute(new Runnable() {
              @Override
              public void run() {
                mAppDatabase.mServiceDao().insert(service);
                mSharedHelper.saveServicePhoneNumber(service.getPhoneNumber());
//        appSharedHelper.saveUser(user);
                userResultListenerListener.onLoad(service);
              }
            });
    ...
    mFirebaseApi.getServiceDatabaseReference().child(service.getKey()).setValue(service);
  }

  public void loadUsers() {
    mFirebaseApi.getServiceUserDatabeseReferance().child(mSharedHelper.getServicePhoneNumber())
        .addListenerForSingleValueEvent(
            new ValueEventListener() {
              @Override
              public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                  for (DataSnapshot serviceSnapshot : dataSnapshot.getChildren()) {
                    DataSnapshot snapshot = serviceSnapshot;// : serviceSnapshot.getChildren()
                ...
                if (!keys.isEmpty()) {
                  for (String uId : keys) {
                    LiveData<Resource<User>> resourceLiveData= loadUser(uId);
                    resourceLiveData.observeForever(new Observer<Resource<User>>() {
                          @Override
                          public void onChanged(
                              @Nullable Resource<User> userResource) {
...
//                  ToastUtils.shortToast("LOADING !!");
                                ...
                                case SUCCESS:
                                  ...
                                  if (mUserList.size() == keys.size()) {
                                    Logger.d(TAG, "LOADED" + mUserList);
                                    mExecutors.diskIO().execute(new Runnable() {
                                      @Override
                                      public void run() {
                                        mAppDatabase.mUserDao().insertAll(mUserList);
                                      }
                                    });
